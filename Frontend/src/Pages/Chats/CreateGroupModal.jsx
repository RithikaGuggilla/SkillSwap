    ...
    <div style={s.overlay} onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div style={s.modal}>
        ...
        <div style={s.field}>
          ...
          <input style={s.input} placeholder="e.g. React Study Group" value={name} onChange={e => setName(e.target.value)} maxLength={50} />
        </div>

        <div style={s.field}>
          <label style={s.label}>Add Members ({selected.length} selected)</label>
          <input style={s.input} placeholder="Search contacts..." value={search} onChange={e => setSearch(e.target.value)} />
        </div>
        ...
        <div style={s.contactList}>
          {filtered.length === 0 ? (
            ...
            filtered.map(c => (
              <div key={c._id} style={s.contactItem(selected.includes(c._id))} onClick={() => toggle(c._id)}>
                <img src={c.picture || `https://api.dicebear.com/7.x/initials/svg?seed=${c.name}`} alt={c.name} style={s.avatar}
                ...
                <div style={s.checkbox(selected.includes(c._id))}>
                  {selected.includes(c._id) && <span style={{ color: "#fff", fontSize: "0.7rem" }}>✓</span>}
                </div>
              </div>
            ))
          )}
...
const s = {
  ...
  header: {
    ...
    padding: "20px 22px 16px", borderBottom: "1px solid #eef2fb",
  },
  title: {
    fontFamily: "'Syne', sans-serif", fontSize: "1.1rem",
    fontWeight: 600, color: "#1a2a4a", margin: 0,
  },
  closeBtn: {
    background: "transparent", border: "none",
    color: "#9aaac8", fontSize: "1rem", cursor: "pointer",
  ...
  label: {
    ...
    textTransform: "uppercase", color: "#6677aa", marginBottom: 7,
  },
  input: {
    width: "100%", background: "#f5f8ff",
    border: "1px solid #dde6f5", borderRadius: 10,
    color: "#1a2a4a", fontFamily: "'DM Mono', monospace",
    fontSize: "0.82rem", padding: "11px 14px", outline: "none",
  },
  contactList: {
    flex: 1, overflowY: "auto", padding: "10px 22px", marginTop: 8,
    scrollbarWidth: "thin", scrollbarColor: "#dde6f5 transparent",
  },
  empty: {
    textAlign: "center", color: "#9aaac8",
    fontFamily: "'DM Mono', monospace", fontSize: "0.75rem", padding: "24px 0",
  },
  contactItem: (selected) => ({
    display: "flex", alignItems: "center", gap: 12,
    padding: "10px 12px", borderRadius: 10, marginBottom: 4,
    background: selected ? "rgba(37,99,235,0.06)" : "transparent",
    border: `1px solid ${selected ? "rgba(37,99,235,0.2)" : "transparent"}`,
    cursor: "pointer", transition: "all 0.15s",
  }),
  avatar: { width: 36, height: 36, borderRadius: "50%", objectFit: "cover", flexShrink: 0 },
  contactName: {
    fontFamily: "'Syne', sans-serif", fontSize: "0.85rem",
    color: "#1a2a4a", fontWeight: 500, marginBottom: 1,
  },
  contactUser: { fontFamily: "'DM Mono', monospace", fontSize: "0.65rem", color: "#9aaac8" },
  checkbox: (checked) => ({
    width: 20, height: 20, borderRadius: "50%",
    border: `1.5px solid ${checked ? "#2563eb" : "#dde6f5"}`,
    background: checked ? "#2563eb" : "transparent",
    display: "flex", alignItems: "center", justifyContent: "center",
    flexShrink: 0, transition: "all 0.15s",
  }),
  footer: { display: "flex", gap: 10, padding: "14px 22px", borderTop: "1px solid #eef2fb" },
  cancelBtn: {
    flex: 1, fontFamily: "'DM Mono', monospace",
    fontSize: "0.65rem", letterSpacing: "0.12em", textTransform: "uppercase",
    padding: "12px", background: "transparent", border: "1px solid #dde6f5",
    color: "#9aaac8", borderRadius: 10, cursor: "pointer",
  },
  createBtn: {
