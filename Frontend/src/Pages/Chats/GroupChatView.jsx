...
const GroupChatView = ({ group, socket, currentUser, onGroupUpdated }) => {
  ...
  // Sync localGroup when prop changes
  ...
  // ── Remove member — uses DELETE /:groupId/remove/:memberId ──
  const handleRemoveMember = async (memberId, memberName) => {
    if (!window.confirm(`Remove ${memberName} from the group?`)) return;
    try {
      await axios.delete(
        `/group/${localGroup._id}/remove/${memberId}`,
        { withCredentials: true }
      );
      toast.success(`${memberName} removed`);
      const updated = {
        ...localGroup,
        members: localGroup.members.filter(m => (m._id || m)?.toString() !== memberId),
      };
  ...
  // ── Leave group — self-removal via DELETE /:groupId/remove/:memberId ──
  const handleLeaveGroup = async () => {
    if (!window.confirm("Are you sure you want to leave this group?")) return;
    try {
      await axios.delete(
        `/group/${localGroup._id}/remove/${currentUser._id}`,
        { withCredentials: true }
      );
  ...
  return (
    <div style={s.wrap}>
      ...
      <div style={s.header}>
        <div style={s.headerLeft} onClick={() => setShowInfo(v => !v)}>
          <div style={s.groupAvatar}>
            {localGroup.picture ? (
              <img src={localGroup.picture} alt={localGroup.name}
          ...
          {messages.map((msg, i) => {
            ...
            return (
              <div key={msg._id || i} style={s.msgRow(mine)}>
                {!mine && (
                  <img
                    src={msg.sender?.picture || `https://api.dicebear.com/7.x/initials/svg?seed=${msg.sender?.name}`}
          ...
          <div style={s.infoPanel}>
            ...
            <div style={s.memberList}>
              ...
              {members.map((m, i) => {
                ...
                const isMemberAdmin = localGroup.admins?.some(
                ...
                return (
                  <div key={i} style={s.memberItem}>
                    ...
                    <div style={{ display: "flex", gap: 4, flexShrink: 0 }}>
                      ...
                      {/* Non-admin can leave from their own row */}
                      {!isAdmin && isMeRow && (
                        <button
                          onClick={handleLeaveGroup}
                          style={s.removeBtn}
            ...
            {/* Admin danger zone */}
            {isAdmin && (
              <div style={{ padding: "12px 14px", borderTop: "1px solid #1a1a1a", marginTop: 8 }}>
      ...
      <div style={s.inputRow}>
        ...
        <input
          style={s.input}
          placeholder="Share notes, links, or resources…"
          value={input}
...
const s = {
  ...
  header: {
    ...
    padding: "14px 20px", borderBottom: "1px solid #1a1a1a", background: "#0f0f0f",
  },
  ...
  groupAvatarText: { color: "#fff", fontSize: "1rem", fontWeight: 700, fontFamily: "'Syne', sans-serif" },
  groupName: { fontFamily: "'Syne', sans-serif", fontSize: "0.95rem", fontWeight: 600, color: "#fff" },
  memberCount: { fontFamily: "'DM Mono', monospace", fontSize: "0.62rem", color: "#666" },
  ...
  senderName: { fontFamily: "'DM Mono', monospace", fontSize: "0.6rem", color: "#888", marginBottom: 3, paddingLeft: 4 },
  bubble: (mine) => ({
    background: mine ? "#ffffff" : "#1a1a1a",
    color: mine ? "#000" : "#ddd",
    borderRadius: mine ? "18px 18px 4px 18px" : "18px 18px 18px 4px",
    padding: "10px 14px",
    display: "flex", alignItems: "flex-end", gap: 8,
    border: mine ? "none" : "1px solid #222",
  }),
  ...
  infoPanel: { width: 280, borderLeft: "1px solid #1a1a1a", background: "#0d0d0d", overflowY: "auto", flexShrink: 0 },
  infoPanelHeader: {
    display: "flex", alignItems: "center", justifyContent: "space-between",
    padding: "16px 16px 12px", borderBottom: "1px solid #1a1a1a",
  },
  infoPanelTitle: { fontFamily: "'Syne', sans-serif", fontSize: "0.85rem", fontWeight: 600, color: "#fff" },
  memberList: { padding: "12px 14px" },
  memberListLabel: { fontFamily: "'DM Mono', monospace", fontSize: "0.58rem", letterSpacing: "0.12em", textTransform: "uppercase", color: "#555", margin: "0 0 10px" },
  memberItem: { display: "flex", alignItems: "center", gap: 8, padding: "10px 0", borderBottom: "1px solid #151515" },
  memberAvatar: { width: 32, height: 32, borderRadius: "50%", objectFit: "cover", flexShrink: 0 },
  memberName: { fontFamily: "'Syne', sans-serif", fontSize: "0.8rem", color: "#ddd", display: "flex", alignItems: "center", gap: 5, flexWrap: "wrap" },
  memberUser: { fontFamily: "'DM Mono', monospace", fontSize: "0.6rem", color: "#555", marginTop: 1 },
  adminBadge: { fontSize: "0.52rem", background: "rgba(255,255,255,0.08)", border: "1px solid rgba(255,255,255,0.15)", borderRadius: 10, padding: "1px 6px", color: "#aaa" },
  youBadge: { fontSize: "0.52rem", background: "rgba(59,180,161,0.15)", border: "1px solid rgba(59,180,161,0.3)", borderRadius: 10, padding: "1px 6px", color: "#3bb4a1" },
  viewBtn: { fontFamily: "'DM Mono', monospace", fontSize: "0.55rem", letterSpacing: "0.08em", textTransform: "uppercase", color: "#888", textDecoration: "none", border: "1px solid #252525", borderRadius: 6, padding: "3px 8px" },
  removeBtn: {
  ...
  inputRow: { display: "flex", gap: 10, padding: "12px 16px", borderTop: "1px solid #1a1a1a", background: "#0d0d0d" },
  input: { flex: 1, background: "#181818", border: "1px solid #252525", borderRadius: 12, color: "#eee", fontFamily: "'Syne', sans-serif", fontSize: "0.85rem", padding: "11px 16px", outline: "none" },
  sendBtn: { background: "#fff", color: "#000", border: "none", borderRadius: 12, padding: "11px 16px", cursor: "pointer", display: "flex", alignItems: "center", transition: "background 0.15s" },
  attachBtn: { display: "flex", alignItems: "center", justifyContent: "center", width: 36, height: 36, borderRadius: "50%", color: "#777", cursor: "pointer", flexShrink: 0 },
};
