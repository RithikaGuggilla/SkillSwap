...
export const UserDetails = asyncHandler(async (req, res) => {
  ...
  const connectionsCount = await Request.countDocuments({
    $or: [
      { sender: user._id, status: "Connected" },
      { receiver: user._id, status: "Connected" },
    ],
  });
 
  return res
    .status(200)
    .json(new ApiResponse(200, { ...user._doc, status, connectionsCount }, "User details fetched successfully"));
});
...
export const getSkillMatches = asyncHandler(async (req, res) => {
  ...
  const matches = await User.find({
    ...
    _id: { $ne: user._id }
  }).select("name username picture rating skillsProficientAt");

  return res.status(200).json(
    new ApiResponse(200, matches, "Skill matches fetched")
  );
...
export const registerUser = async (req, res) => {
  ...
  const newUser = await User.create({
    ...
    password: req.user.password ?? null,  // ← carries hashed password for email sign-ups; null for Google
  });
...
export const discoverUsers = asyncHandler(async (req, res) => {
  ...
  const machineLearningSkills = [
    "Python", "Natural Language Processing", "Deep Learning", "PyTorch", "Machine Learning",
  ];

  const users = await User.find({ _id: { $ne: req.user._id }, isBanned: { $ne: true } });
  ...
  users.forEach((user) => {
    if (addedUsers.has(user._id.toString())) return;

    if (
      user.skillsProficientAt.some(skill => req.user.skillsToLearn.includes(skill)) &&
      ...
      user.skillsProficientAt.some(skill => machineLearningSkills.includes(skill)) &&
      mlUsers.length < 5
    ) {
      mlUsers.push(user);
      addedUsers.add(user._id.toString());
  ...
  return res.status(200).json(
    new ApiResponse(200, { forYou: usersToLearn, webDev: webDevUsers, ml: mlUsers, others: otherUsers }, "Users fetched successfully")
  );
});
